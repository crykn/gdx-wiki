...
feature_row3:
  - image_path: /assets/images/features/ecosystem.png
    ...
    btn_class: "btn--primary"
list1:
  - "[Streaming music](/wiki/streaming-music) and [sound effect playback](/wiki/sound-effects) for WAV, MP3 and OGG"
  - "Direct access to audio device for [PCM sample playback](/wiki/playing-pcm-audio) and [recording](/wiki/recording-pcm-audio)"
list2:
  - "Abstractions for [mouse, keyboard, touchscreen](/wiki/mouse,-touch-and-keyboard), [controllers](https://github.com/libgdx/gdx-controllers), [accelerometer](/wiki/accelerometer), [gyroscope](/wiki/gyroscope) and [compass](/wiki/compass)"
  - "[Gesture detection](/wiki/gesture-detection) (recognising taps, panning, flinging and pinch zooming)"
list3:
  - "[Matrix, vector and quaternion](/wiki/vectors,-matrices,-quaternions) classes; accelerated via native C code where possible (if you are interested in that, also note our [gdx-jnigen](/wiki/jnigen) project)"
  - "[Bounding shapes and volumes as well as a Frustum class for picking and culling](/wiki/circles,-planes,-rays,-etc.)"
  ...
  - "Common [interpolators](/wiki/interpolation), different [spline implementations](/wiki/path-interface-and-splines), concave polygon triangulators and more"
  - "2D physics: JNI wrapper for the popular [Box2D physics](/wiki/box2d) (see also [Box2DLights](https://github.com/libgdx/box2dlights)). Alternatively, you can take a look at [jbump](https://github.com/implicit-invocation/jbump) for a simpler physics implementation."
  - "3D physics: JNI Wrapper for [Bullet physics](/wiki/bullet-physics)"
list4:
  ...
  - "[JSON](/wiki/reading-and-writing-json) and [XML](/wiki/reading-and-writing-xml) serialisation"
  - "Custom, [optimised collections](/wiki/collections), with primitive support"
list5:
  ...
  - "Easy integration of [AdMob](/wiki/admob-in-libgdx)"
list6:
  - "Rendering through [OpenGL ES 2.0/3.0](/wiki/opengl-es-support) on all platforms"
  - a:
    title: "**Low-Level OpenGL helpers:**"
    items:
      - "Vertex arrays and vertex buffer objects"
      - "[Meshes](/wiki/meshes)"
      - "[Textures](/wiki/textures-and-textureregions)"
      - "[Framebuffer objects](/wiki/frame-buffer-objects)"
      - "[Shaders](/wiki/shaders), integrating easily with meshes"
      ...
      - "Simple [shape rendering](/wiki/rendering-shapes)"
  ...
  - b:
    ...
    items:
      ...
      - "High-performance [sprite](/wiki/spritebatch%2C-Textureregions%2C-and-Sprites) batching and caching"
      - "[Texture atlases](/wiki/using-textureatlases), with whitespace stripping support. Either generated [offline](/wiki/packing-atlases-offline) or [at runtime](/wiki/packing-atlases-at-runtime)"
      - "[Bitmap fonts](/wiki/bitmap-fonts). Either generated offline or [loaded from TTF files](/wiki/gdx-freetype)"
      - "[2D Particle system](/wiki/2d-particleeffects)"
      - "[TMX tile map support](/wiki/tile-maps)"
      - "[2D scene-graph API](/wiki/scene2d)"
  - c:
    title: "**A Powerful UI Solution:**"
    items:
      - "[2D UI library](/wiki/scene2d.ui), based on scene-graph API"
      - "A plethora of [official](/wiki/scene2d.ui#widgets) and third-party widgets"
      - "Fully [skinnable](/wiki/skin); [Composer](https://github.com/raeleus/skin-composer) for creating UI skins"
  - d:
    title: "**High-Level 3D APIs:**"
    items:
      - "Decal batching, for 3D billboards or [particle systems](/wiki/3d-particle-effects)"
      - "Basic loaders for Wavefront OBJ and MD5"
      - "[3D rendering API](/wiki/3d-graphics) with materials, animation and lighting system and support for loading FBX models via fbx-conv"
      - "Third-party support for [GLTF 2.0](https://github.com/mgsx-dev/gdx-gltf)"
      - "Rudimentary [VR support](/wiki/virtual-reality-%28VR%29)"
